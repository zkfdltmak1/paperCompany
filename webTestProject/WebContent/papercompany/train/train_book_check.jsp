...
<link type="text/css" rel="stylesheet" href="/webTestProject/style/css/booking/train/train_check.css"/>
	 ...
	 $(function(){
		...
		$("#payCheck").bind("click",function(){
			...
			if(confirm("결제를 하시겠습니까?")){
				...
				$("#train_detail_form").attr( "action", "../train/train.do" );
				$("#train_detail_form").submit();
			}
...
<body>
	...
	<div id="main" >
		...
		<table class="train_table_id">
			...
			<tr>
				<td>${requestScope.train_date }</td>
				<td>${requestScope.train_type }</td>
		...
		<form id="train_detail_form" name="train_detail_form">
			...
			<div>
				<table class="train_table_id">
					...
					<c:forEach var="tList" items="${requestScope.tList}">
						<tr>
							<td>${tList.seat_seat}</td>
							<td>${tList.seat_number}</td>
							<td>${tList.people}</td>
							<td>${tList.charge}</td>
						</tr>
					</c:forEach>
